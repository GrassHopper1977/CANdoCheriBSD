//------------------------------------------------------------------------------
//  TITLE :- CRC Calculation routines - CRC.c
//  AUTHOR :- Martyn Brown
//  DATE :- 25/04/14
//
//  DESCRIPTION :- CRC 16 calculations.
//
//  UPDATES :-
//  25/04/14 CRC.pas v4.0 converted to 'C' for Linux library
//
//  LICENSE :-
...
// CalculateCRC
//
// Calculate the CRC for the data series supplied.
//
// Returns
//    CRC value
//--------------------------------------------------------------------------
void CalculateCRC(const unsigned char * DataPointer, unsigned int NoOfBytes, unsigned int * CRCValue)
{
  unsigned int ByteNo;
  const unsigned char * BufferPointer;

  BufferPointer = DataPointer;
  for (ByteNo = 1; ByteNo <= NoOfBytes; ByteNo++)
  {
    *CRCValue = ((*CRCValue >> 8) & 0xFF) ^ CRC16Table[*BufferPointer ^ (*CRCValue & 0xFF)];
    BufferPointer++;
  }
}
//--------------------------------------------------------------------------
