...
void CANdoConnect(void)
{
  ...
  if (Status == CANDO_SUCCESS)
  {
		if (NoOfDevices > 1)
		{
			...
			printf("\n %d CANdo Found -", NoOfDevices);
			for (DeviceNo = 0; DeviceNo < NoOfDevices; DeviceNo++)
			{
				if (CANdoDevices[DeviceNo].HardwareType == CANDO_TYPE_CANDO)
					printf("\n %d = CANdo S/N %s", DeviceNo + 1, CANdoDevices[DeviceNo].SerialNo);
				else
				if (CANdoDevices[DeviceNo].HardwareType == CANDO_TYPE_CANDOISO)
					printf("\n %d = CANdoISO S/N %s", DeviceNo + 1, CANdoDevices[DeviceNo].SerialNo);
				else
				if (CANdoDevices[DeviceNo].HardwareType == CANDO_TYPE_CANDO_AUTO)
					printf("\n %d = CANdo AUTO S/N %s", DeviceNo + 1, CANdoDevices[DeviceNo].SerialNo);
				else
					printf("\n %d = Type Unknown?", DeviceNo + 1);
			}
		}
      ...
      {
        // Connection open
        DeviceType = CANdoDevices[0].HardwareType;
...
void CANdoStart(void)
{
  ...
  if (CANdoUSB.OpenFlag)
  {
    ...
    if (CANdoSetBaudRate(&CANdoUSB, 0, 1, 7, 7, 2, 0) == CANDO_SUCCESS)
    {
  ...
  if (RunState)
    printf("\n\n CANdo started @250k CAN baud rate");
}
//--------------------------------------------------------------------------
// CANdoStop
//
